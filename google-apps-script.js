...
function doPost(e) {
  try {
    ...
    if (formData.fullName && formData.phoneNumber) {
      ...
      sendConfirmationEmail(formData.fullName, formData.phoneNumber);
      ...
      // Get the Google Sheet for contacts (you can create a separate sheet or use the same one)
      const sheet = SpreadsheetApp.openById('YOUR_GOOGLE_SHEET_ID').getActiveSheet();
...
function sendConfirmationEmail(name, phone) {
  try {
    ...
    // Send email to the applicant (you'll need to collect email in the form)
    // GmailApp.sendEmail(email, subject, body);
    
    // Send notification to ISC team
    GmailApp.sendEmail('internationalstudentsclub@psu.edu.sa', 
